...
export default class attackAction extends Phaser.Physics.Arcade.Sprite {
    ...
    update() {
        ...
        if(this.me.cursors.space.isDown && this.enemy.keys.keyS.isUp){
            this.countSpace ++;
        ...
        if (this.enemy.keys.keyS.isDown){
            this.attackPermission = false;
        }

        //spaceを押して、meとenemyの距離が65以下の時に攻撃ができる
        if (this.me.cursors.space.isDown && Math.abs(this.me.x-this.enemy.x)<50 && Math.abs(this.me.y-this.enemy.y)<50 && !this.me.lastSpaceDown)
        { 
        ...
        //shiftを押すと飛び攻撃
        //keySを押している間は攻撃無効
        if(this.weapon.cursors.shift.isDown && this.enemy.keys.keyS.isUp){
